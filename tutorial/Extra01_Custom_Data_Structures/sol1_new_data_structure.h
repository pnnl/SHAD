#ifndef INCLUDE_SHAD_DATA_STRUCTURES_MY_NEWEST_DS_H_
#define INCLUDE_SHAD_DATA_STRUCTURES_MY_NEWEST_DS_H_
...
class MyNewDS : public AbstractDataStructure<MyNewDS<T>> {
 public:
  ...
  std::vector<T> gather() {
    ...
    rt::Handle h;
    for (auto loc : shad::rt::allLocalities()) {
      rt::asyncExecuteAtWithRet(h, loc,
        [](rt::Handle&, const ObjectID &oid, T* res) {
          auto ptr = MyNewDS<T>::GetPtr(oid);
          *res = static_cast<T>(*ptr);
        },  
        oid_, &res[idx]);
      ++idx;
    }
...
#endif  // INCLUDE_SHAD_DATA_STRUCTURES_MY_NEWEST_DS_H_