...
// apply and foreach
  ...
  auto setPtr = shad::Set<float>::Create(kNumValues* rt::numLocalities());
  auto setOID = setPtr->GetGlobalID();

  auto AsyncInsertLambda = [](rt::Handle &h, const uint64_t &key, float &value,
                         shad::Set<float>::ObjectID &oid) {
    if (value > kMagicNumber) {
      auto setPtr = shad::Set<float>::GetPtr(oid);
      setPtr->BufferedAsyncInsert(h, value);
    }
  };
  mapPtr->AsyncForEachEntry(h, AsyncInsertLambda, setOID);
  rt::waitForCompletion(h);
  setPtr->WaitForBufferedInsert();
  shad::Set<float>::Destroy(setOID);
