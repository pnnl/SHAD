...
class IDCmp {
 public:
  bool operator()(const T *first, const T *sec) const {
    return *first != *sec;
  }
};
...
class DefaultEdgeIndexStorage {
 public:
  ...
  struct LocalEdgeListChunk {
    LocalEdgeListChunk(size_t _numDest,
                       bool _ow, DestT* _dest)
      : numDest(_numDest)
      , overwrite(_ow) {
  ...
  struct ElementInserter {
    void operator()(NeighborsStorageT * const lhs,
                    const  NeighborsStorageT&) {
    }
  ...
  void ForEachAttributedVertexNeighbor(const SrcT& src,
                                       ApplyFunT &&function,
                                       Args... args);

  template<typename ApplyFunT, typename ...Args>
  void AsyncForEachAttributedVertexNeighbor(rt::Handle& handle,
                                            const SrcT& src,
                                            ApplyFunT &&function,
                                            Args... args);
  ...
  template<typename ApplyFunT, typename ...Args>
  void AsyncForEachAttributedVertex(rt::Handle& handle,
                                    ApplyFunT &&function,
                                    Args... args);

  SrcAttributesT* GetVertexAttributes(const SrcT &src) {
    printf("WARNING: Trying to lookup attributes from"
  ...
  static void CallVertexAttributesApplyFun(DefaultEdgeIndexStorage<SrcT, DestT,
                                           NeighborListStorageT>* stPtr,
                                           const SrcT &key,
                                           ApplyFunT function,
                                           std::tuple<Args...> & args,
...
class LocalEdgeIndex {
 public:
  template<typename, typename> friend class LocalSet;
  ...
  void AsyncErase(rt::Handle &handle, const SrcT& src, const DestT& dest) {
    auto edgeList = edges_.edgeList_.Lookup(src);
    if (edgeList != nullptr)
      edgeList->AsyncErase(handle, dest);
  }
  ...
  template<typename ApplyFunT, typename ...Args>
  void ForEachNeighbor(const SrcT& src,
                       ApplyFunT &&function,
                       Args&... args) {
    auto edgeList = edges_.edgeList_.Lookup(src);
    if (edgeList == nullptr) {
  ...
  void AsyncForEachAttributedVertexNeighbor(rt::Handle& handle,
                                            ...
                                            ApplyFunT &&function,
                                            Args&... args) {
    edges_.AsyncForEachAttributedVertexNeighbor(handle, function,
                                                src, args...);
  }
