...
class AttrEdgesPair {
 public:
  ...
  void Erase(const DestT &dest) {
    neighbors_.Erase(dest);
  }
  void AsyncErase(rt::Handle &handle, const DestT &dest) {
    neighbors_.Erase(handle, dest);
  }

  void Insert(const DestT &dest) {
    neighbors_.Insert(dest);
  }
 ...
 public:
  ...
  struct ElementInserter {
    ...
    static void Insert(NeighborsStorageT * const lhs,
      ...
      if (values.overwrite)
        lhs->neighbors_.Reset(values.numValues);
    ...
    static void Insert(NeighborsStorageT * const lhs,
      ...
      if (chunk.overwrite)
        lhs->neighbors_.Reset(chunk.numDest);
  ...
  static void CallVertexAttributesApplyFun(AttributedEdgeIndexStorage<SrcT,
                                           DestT, SrcAttrT,
...
template <typename SrcT, typename SrcAttrT, typename DestT>
using AttributedEdgeIndex = EdgeIndex<SrcT,
                                      DestT,
                     AttributedEdgeIndexStorage<SrcT, DestT, SrcAttrT>>;
