...
#ifndef INCLUDE_SHAD_EXTENSIONS_HYPERGRAPH_LIBRARY_INDEX_H_
#define INCLUDE_SHAD_EXTENSIONS_HYPERGRAPH_LIBRARY_INDEX_H_
...
/// @brief The Index data structure.
///
/// SHAD's Index is a thread-safe, associative container,
...
/// @tparam StorageT Index local storage. Default is a map of sets.
template <typename SrcT, typename DestT,
          typename StorageT = v2DefaultEdgeIndexStorage<SrcT, DestT>>
class Index
    : public AbstractDataStructure<Index<SrcT, DestT, StorageT>> {
  template <typename>
 ...
 public:
   using value_type = std::pair<SrcT, StorageT>;
// defs
//   using iterator =
...
//---------------
  ...
  using ObjectID = typename AbstractDataStructure<
      Index<SrcT, DestT, StorageT>>::ObjectID;
  using EdgeListPtr = typename AbstractDataStructure<
      Index<SrcT, DestT, StorageT>>::SharedPtr;
  using SrcType = SrcT;
  using DestType = DestT;
  using LIdxT = LocalIndex<SrcT, DestT, StorageT>;
  using IdxT = Index<SrcT, DestT, StorageT>;
  struct EntryT {
  ...
  using BuffersVector =
      typename impl::BuffersVector<EntryT, Index<SrcT, DestT, StorageT>>;
  ...
  static IndexPtr Create(const size_t numVertices);
#endif
  ...
  void WaitForBufferedInsert() {
    auto flushLambda_ = [](const ObjectID &oid) {
      auto ptr = Index<SrcT, DestT, StorageT>::GetPtr(oid);
  ...
  LocalIndex<SrcT, DestT, StorageT> *GetLocalIndexPtr() {
    return &localIndex_;
  }
 ...
 private:
  ObjectID oid_;
  LocalIndex<SrcT, DestT, StorageT> localIndex_;
  ...
  struct EdgeListChunk {
    ...
    typename Index<SrcT, DestT, StorageT>::ObjectID oid;
    SrcT src;
 ...
 protected:
  Index(ObjectID oid, const size_t numVertices)
...
inline size_t Index<SrcT, DestT, StorageT>::Size() const {
...
template <typename SrcT, typename DestT, typename StorageT>
inline size_t Index<SrcT, DestT, StorageT>::NumEdges() {
...
template <typename SrcT, typename DestT, typename StorageT>
inline size_t Index<SrcT, DestT, StorageT>::GetDegree(const SrcT &src) {
  ...
  if (targetLocality == rt::thisLocality()) {
    ...
    auto degreeLambda = [](const std::tuple<ObjectID, SrcT> &args,
                           size_t *res) {
      auto ptr = Index<SrcT, DestT, StorageT>::GetPtr(std::get<0>(args));
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::Insert(const SrcT &src,
                                                     const DestT &dest) {
  size_t targetId = shad::hash<SrcT>{}(src) % rt::numLocalities();
  rt::Locality targetLocality(targetId);
  ...
  if (targetLocality == rt::thisLocality()) {
    ...
    auto insertLambda = [](const InsertArgs &args) {
      auto ptr = Index<SrcT, DestT, StorageT>::GetPtr(args.oid);
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::InsertEdgeList(
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::AsyncInsertEdgeList(
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::AsyncInsert(rt::Handle &handle,
                                                          const SrcT &src,
                                                          const DestT &dest) {
  size_t targetId = shad::hash<SrcT>{}(src) % rt::numLocalities();
  rt::Locality targetLocality(targetId);
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::Erase(const SrcT &src,
                                                    const DestT &dest) {
  size_t targetId = shad::hash<SrcT>{}(src) % rt::numLocalities();
  rt::Locality targetLocality(targetId);
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::AsyncErase(rt::Handle &handle,
                                                         const SrcT &src,
                                                         const DestT &dest) {
  size_t targetId = shad::hash<SrcT>{}(src) % rt::numLocalities();
  rt::Locality targetLocality(targetId);
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::BufferedInsert(
...
template <typename SrcT, typename DestT, typename StorageT>
inline void Index<SrcT, DestT, StorageT>::BufferedAsyncInsert(
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::ForEachVertex(ApplyFunT &&function,
                                                     Args &... args) {
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::AsyncForEachVertex(rt::Handle &handle,
                                                          ApplyFunT &&function,
                                                          Args &... args) {
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::ForEachNeighbor(const SrcT &src,
                                                       ApplyFunT &&function,
                                                       Args &... args) {
  size_t targetId = shad::hash<SrcT>{}(src) % rt::numLocalities();
  rt::Locality targetLocality(targetId);
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::AsyncForEachNeighbor(
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::ForEachEdge(ApplyFunT &&function,
                                                   Args &... args) {
  using FunctionTy = void (*)(const SrcT &src, const DestT &dest, Args &...);
  FunctionTy fn = std::forward<decltype(function)>(function);
  using feArgs = std::tuple<ObjectID, FunctionTy, std::tuple<Args...>>;
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::AsyncForEachEdge(rt::Handle &handle,
                                                        ApplyFunT &&function,
                                                        Args &... args) {
  using FunctionTy = void (*)(rt::Handle & handle, const SrcT &src,
                              const DestT &dest, Args &...);
...
template <typename SrcT, typename DestT, typename StorageT>
bool Index<SrcT, DestT, StorageT>::GetVertexAttributes(
  ...
  if (targetLocality == rt::thisLocality()) {
    ...
    auto lookupLambda = [](const LookupArgs &args, LookupResult *res) {
      auto eiPtr = Index<SrcT, DestT, StorageT>::GetPtr(args.oid);
...
template <typename SrcT, typename DestT, typename StorageT>
template <typename ApplyFunT, typename... Args>
void Index<SrcT, DestT, StorageT>::VertexAttributesApply(
...
#endif  // INCLUDE_SHAD_EXTENSIONS_HYPERGRAPH_LIBRARY_INDEX_H_
