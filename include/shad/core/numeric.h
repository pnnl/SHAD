...
template <class InputIt, class OutputIt, class BinaryOperation>
OutputIt adjacent_difference(distributed_sequential_tag&& policy,
                             InputIt first, InputIt last,
                             OutputIt d_first, BinaryOperation op) {
  return impl::adjacent_difference(first, last, d_first, op);
}
        ...
        [](rt::Handle&, const std::tuple<InputIt, InputIt, OutputIt,
          ...
          if (rt::thisLocality() == std::get<3>(args)) {
            *d_first = acc;
            ...
            --it;
            d_first += (std::distance(gbegin, it));
            value_t val = *d_first;
            *d_first = op(*begin, *it);
          } 
          while (++begin != end) {
...
OutputIt adjacent_difference(InputIt first, InputIt last,
  ...
  return impl::adjacent_difference(first, last, d_first,
                                   std::minus<value_t>());
}
...
adjacent_difference(ExecutionPolicy&& policy,
                             InputIt first,
                             InputIt last, OutputIt d_first) {
  using value_t = typename distributed_iterator_traits<InputIt>::value_type;
