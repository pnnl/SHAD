...
class LocalHashmap {
 ...
 public:
  ...
  using iterator = lmap_iterator<LocalHashmap<KTYPE, VTYPE,
                                              KEY_COMPARE, INSERTER>,
                                 std::pair<KTYPE, VTYPE>>;
  using const_iterator = lmap_iterator<LocalHashmap<KTYPE, VTYPE,
                                                    KEY_COMPARE, INSERTER>,
                                       const std::pair<KTYPE, VTYPE>>;
  /// @brief Constructor.
  ...
  iterator end() {
    return iterator::lmap_end(numBuckets_);
  }
  ...
  const_iterator cend() {
    return const_iterator::lmap_end(numBuckets_);
  }
...
class lmap_iterator : public std::iterator<std::forward_iterator_tag, T>{
 ...
 public:
  ...
  lmap_iterator(const LMap* mapPtr, size_t bId, size_t pos,
                Bucket* cb, Entry* ePtr) :
                      mapPtr_(mapPtr), bucketId_(bId),
  ...
  T operator*() {
    return T(entryPtr_->key, entryPtr_->value);
  }
